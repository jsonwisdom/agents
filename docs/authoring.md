...
Plugin content in this repo ships to **six generated harnesses** plus Claude Code: OpenAI Codex CLI, Cursor, OpenCode, Gemini CLI, GitHub Copilot, and Qwen Code. Claude Code is the source-of-truth. The adapter framework handles per-harness
...
1. **Context file is a table of contents, not an encyclopedia.** Keep `AGENTS.md`,
   `CLAUDE.md`, `GEMINI.md`, and `QWEN.md` under ~150 lines / ~500 tokens. Detail belongs in
...
> `.cursor-plugin/`, `gemini-extension.json`, `qwen-extension.json`) point at the source `plugins/` and are checked in.
...
| Source field | Codex | Cursor | OpenCode | Gemini | Copilot | Qwen |
|---|---|---|---|---|---|---|
| `model: fable` | `gpt-5.5` | `inherit` | `anthropic/claude-fable-5` | `gemini-2.5-pro` | `claude-fable-5` | `inherit` |
| `model: opus` | `gpt-5.5` | `inherit` | `anthropic/claude-opus-4-8` | `gemini-2.5-pro` | `claude-opus-4.8` | `inherit` |
| `model: sonnet` | `gpt-5.4-mini` | `inherit` | `anthropic/claude-sonnet-5` | `gemini-2.5-pro` | `claude-sonnet-5` | `inherit` |
| `model: haiku` | `gpt-5.4-mini` | `inherit` | `anthropic/claude-haiku-4-5` | `gemini-2.5-flash` | `claude-haiku-4.5` | `fast` |
| `model: inherit` | `gpt-5.5` | `inherit` | `anthropic/claude-sonnet-5` | `gemini-2.5-pro` | `claude-sonnet-5` | `inherit` |
...
Qwen Code maps `fable`/`opus`/`sonnet`/`inherit` to portable `inherit` and `haiku` to `fast`.
...
This is the canonical Anthropic SKILL.md pattern. Codex, Cursor, OpenCode, Gemini, and Qwen all
honor `references/`.
...
| Per-agent tool allowlist | Honored only on Claude Code/Gemini/OpenCode/Qwen. Cursor and Codex have coarser models. |
| Slash commands | Codex converts to skills. Gemini transpiles to TOML. Copilot emits `.copilot/commands/` prompt files. Qwen emits Markdown under `.qwen/commands/` with `{{args}}`. |
| Marketplace registry | Only Claude Code and Cursor have one. Gemini and Qwen install by URL/path; Codex/OpenCode have no marketplace. |
