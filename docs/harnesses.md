...
| **Qwen Code** | supported | committed: `qwen-extension.json`; gitignored: `.qwen/skills/`, `.qwen/agents/`, `.qwen/commands/` |

## Capability matrix

| Capability | Claude Code | Codex | Cursor | OpenCode | Gemini | Qwen |
|---|---|---|---|---|---|---|
| Skills (SKILL.md native) | ✅ | ✅ | ✅ via `.claude/` | ✅ via `.opencode/skills/` | ✅ (auto-discovered) | ✅ via `.qwen/skills/` |
| Subagents (markdown native) | ✅ | TOML format | ✅ via `.claude/` | ✅ (different frontmatter) | ✅ (April 2026 spec) | ✅ (markdown; `inherit`/`fast`) |
| Slash commands | ✅ | converted to skills | ✅ | ✅ | TOML at `commands/` | Markdown at `.qwen/commands/` |
| Plugin marketplace | ✅ | — | ✅ (2.5+) | — | — (direct URL install) | — (direct path/URL install) |
| Parallel subagents | ✅ | ✅ | ✅ | ✅ | ✅ (April 2026) | ✅ (fork subagents) |
| Per-agent tool allowlist | ✅ (`tools:`) | only `sandbox_mode` | only `readonly:` | ✅ (`permission:` block) | ✅ (`tools:`) | ✅ (`tools:`, Qwen names) |
| `TodoWrite` tool | ✅ | — | — | ✅ | — | `todo` (not 1:1) |
| `Task`/`Agent` spawn tool | ✅ | name in prose | ✅ | ✅ (`task`) | `@agent` syntax | `agent` tool |
| MCP servers | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| Lifecycle hooks | ✅ | — | — | ✅ (TS plugins) | — | ✅ |
| Context file | `CLAUDE.md` | `AGENTS.md` (32 KiB cap) | `AGENTS.md` | `AGENTS.md` / `~/.claude/CLAUDE.md` | `GEMINI.md` | `QWEN.md` (setup); runtime `AGENTS.md` |
...
| Tool name case | CamelCase (`Read`) | action verbs (no tool vocab) | lowercase | lowercase (strict) | lowercase (Gemini names) | lowercase (Qwen names) |
| Bare model aliases | ✅ (`fable`/`opus`/`sonnet`/`haiku`) | mapped to GPT-5.x family | use `inherit` | full provider/model-id | mapped to `gemini-2.5-*` | `inherit` / `fast` |
...
- **`TodoWrite`** — native progress tracking. (Not available in Codex / Cursor / Gemini; Qwen has a `todo` tool that is not a 1:1 port.)
...
| Source pattern | Codex | Cursor | OpenCode | Gemini | Qwen |
|---|---|---|---|---|---|
| `tools: Read, Grep` (agent allowlist) | dropped; `sandbox_mode = "read-only"` heuristic | dropped (Cursor doesn't honor) | converted to `permission:` deny block | passed through | remapped to `read_file`, `grep_search` |
| `color: blue` (agent) | dropped | dropped | dropped | dropped | dropped |
| `model: opus` (agent) | mapped to `gpt-5.5` | rewritten to `inherit` | rewritten to `anthropic/claude-opus-4-8` | mapped to `gemini-2.5-pro` | rewritten to `inherit` |
| `model: fable` (agent) | mapped to `gpt-5.5` | rewritten to `inherit` | rewritten to `anthropic/claude-fable-5` | mapped to `gemini-2.5-pro` | rewritten to `inherit` |
| `TodoWrite` in body | no equivalent — leave as-is | no equivalent — leave as-is | works as-is | no equivalent | rewritten to `todo` |
| Skill body > 8 KB | split into `references/details.md` | passed through | passed through | passed through | passed through |
| Agent named `worker` | namespaced to `<plugin>__worker` | passed through | passed through | passed through | namespaced to `<plugin>__worker` |
| Slash command (`commands/<x>.md`) | converted to skill | passed through | rewritten to `.opencode/commands/` | TOML at `commands/<plugin>/<x>.toml` | Markdown at `.qwen/commands/<plugin>/<x>.md` |
...
qwen-extension.json                    # Qwen extension manifest (contextFileName: AGENTS.md)
QWEN.md                                # Qwen setup guide (not injected at runtime)
```
...
.qwen/skills/, .qwen/agents/, .qwen/commands/
```
...
- **Qwen Code** — same lean tradeoff as Gemini (generated trees are gitignored). Clone the
  repo, then `make generate HARNESS=qwen` and `qwen extensions install .` (local path).
  See [`QWEN.md`](../QWEN.md).
